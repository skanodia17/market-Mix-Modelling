...
Adstock: Adstock of each of the ‘commercial spends’ (Since we don’t have TRP/ Impression information, we will assume spend is directly proportional to the impression)
...
This model assumes that as the data increase, so does the seasonal pattern. Most time series plots exhibit such a pattern. In this model, the trend and seasonal components are multiplied and then added to the error component. Multiplicative model’s benefits include synergistic impact of marketing activities, independence of shapes of transformations independent variable scan take due to coefficient values and coefficients directly provide simple elasticity indications.
...
A distributed-lag model is a dynamic model in which the effect of a regressor x on y occurs over time rather than all at once. The distributed lag model is a model with multiple lagged values of both the dependent variable and the independent variable. This model is very general and can capture a whole range of carryover effects. Indeed, the Koyck model can be considered a special case of distributed lag model with only one lagged value of the dependent variable. The distributed lag model overcomes two of the problems with the Koyck model. First, it allows for decay functions, which are nonmonotonic or humped shaped. Second, it can partly separate out the carryover effects of different independent variables.

Deployment
I have used R for building the models and shiny for deploying this model. 
Exploratory Data Analysis 
Models and Elasticity Analysis
